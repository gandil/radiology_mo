...
def svm_model(X_train, Y_train, X_test, Y_test):
    ...
    svm_classifier = SVC(kernel='rbf', gamma=.001, C=10)
...
def calculate_all_acc_parameters(predicted_labels, y_true):
    return accuracy_score(y_true, predicted_labels), f1_score(y_true, predicted_labels), recall_score(y_true, predicted_labels), precision_score(
        y_true, predicted_labels)


def ten_cross_validation(data_X, data_Y,model_name='svm', folds=10):
...
pd.DataFrame(np.array(ten_cross_validation(X, Y, folds=10)).T,columns=['Fold_Number', 'Accuracy', 'F1_Score', 'Recall', 'Precision']).to_csv('Results.csv', index=False)